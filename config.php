...
class Userfunction{
	...
	public function logueado(){
		...
		if(isset($_COOKIE['taller_pass']) && isset($_COOKIE['taller_user']) && (int)$_COOKIE['taller_user']>0 && !empty($_COOKIE['taller_pass'])){
									
			$sql = 'SELECT PASS FROM USUARIOS WHERE ID='.$_COOKIE['taller_user'];

			$select_assoc_query = mysqli_query($this->con, $sql);
		
			$row = mysqli_fetch_array($select_assoc_query);
			
			if($row[0]==$_COOKIE['taller_pass']){
	...
	public function idioma($accion,$idioma){
		...
		if(!empty($idioma)){
			...
			$sql = 'SELECT TEXTO FROM '.$idioma." WHERE ACCION='".$accion."'";
	...
	public function ver_nivel($user){
		
		$resultado=-1;
		
		if(!empty($user)){
		
		$sql = "SELECT NIVEL FROM USUARIOS WHERE ID=$user";
	...
	public function loguear($user,$pass){
		...
		$sql = "SELECT PASS,ID,NOMBRE FROM USUARIOS WHERE NOMBRE='".$user."'";
		...
		if(mysqli_num_rows($select_assoc_query) ==1 && ($user==$row[2] && compare_passwords($pass, $row[0]))){

				$respuesta=$row[1].' '.$row[0];
